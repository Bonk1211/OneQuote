...
async def chat_quote(
    ...
    """Stream SSE events with real-time AI state updates, then a final response."""

    async def _generate() -> AsyncGenerator[str, None]:
        # ── State: checking_sources ──────────────────────────────────
        yield _sse_event("ai_state", {"state": "checking_sources"})
        ...
        if not overheads_resp.data:
            yield _sse_event("error", {"message": "Please set up your business overheads first."})
            return
        ...
        yield _sse_event("ai_state", {"state": "loading_context"})
        ...
        # ── State: thinking ──────────────────────────────────────────
        yield _sse_event("ai_state", {"state": "thinking"})
        ...
        # ── State: parsing ───────────────────────────────────────────
        yield _sse_event("ai_state", {"state": "parsing"})
        ...
        if parsed is None:
            ...
            yield _sse_event("ai_state", {"state": "idle"})
            yield _sse_event("response", ChatResponse(
                message=clean_msg,
                conversation_id=conversation_id,
            ).model_dump())
            return
        ...
        # ── State: profitability ─────────────────────────────────────
        yield _sse_event("ai_state", {"state": "profitability"})
        ...
        # ── State: saving ────────────────────────────────────────────
        yield _sse_event("ai_state", {"state": "saving"})
